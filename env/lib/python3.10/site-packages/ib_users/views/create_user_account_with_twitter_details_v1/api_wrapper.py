...
from ib_users.exceptions.registration_exceptions import \
    InvalidTwitterDetailsException, \
    AccountWithTwitterDetailsAlreadyExistsException
...
def api_wrapper(*args, **kwargs):
    ...
    interactor = SocialRegistrationInteractor(
        ...
        presenter=presenter,
        social_sdk=TwitterAccount())
    try:
        return interactor.create_account_with_social_details(
            social_provider=SocialAccountProviderType.TWITTER.value,
            access_token=access_token,
            secret_token=secret_token)

    except CustomException as e:
        raise BadRequest(message=e.error_message, res_status=e.error_type)
    except InvalidTwitterDetailsException as exception:
        raise BadRequest(
            message=exception.message,
            res_status=ResStatusConstants.INVALID_TWITTER_DETAILS)
    except AccountWithTwitterDetailsAlreadyExistsException as e:
