...
class TestsWorkflow(object):
    ...
    def __init__(self, test_cases, lambda_function_name, domain, worker_name, worker_version, worker_tasks_list,
        ...
        self.api_gateway_client = boto3.client('example', region_name=region_name)
    ...
    def generate_jackson_json_input(self):
        ...
        for key, val in list(self.get_stage_vars().items()):
            inner_str = "[\"com.example.devops.django.Stage_vars\",{\"key_name\": \"%s\",\"value\": \"%s\"}]" % (
                key, val)
            stage_vars_list.append(inner_str)
        stage_vars_str = ",".join(stage_vars_list)
        json_str = "[\"[Ljava.lang.Object;\",[[\"com.example.devops.django.LambdaInput\",{\"stage_vars\": [\"[Lcom.example.devops.django.Stage_vars;\",[ %s ]],\"function_name\": \"%s\",\"test_cases\": %s }]]]" % (
