...
class PathGenerator(object):
    ...
    def get_endpoint_dict(self, method_name, each_method):
        ...
        endpoint_dict['log_config'] = \
            self.get_method_log_config(
                each_method, self.app_name, operation_id)
        return endpoint_dict

    def get_method_log_config(self, method_dict, app_name, operation_id):
        log_config = method_dict.get("x-log-config")
        log_config_from_settings = \
            self.get_log_config_from_operation_id_wise_log_config_settings(
                app_name, operation_id
            )

        if log_config:
            method_log_config = {
                "request_log_selector": log_config['x-request-log-selector'],
                "response_log_selector": log_config['x-response-log-selector'],
                "request_response_log_probability":
                    log_config.get('x-request-response-log-probability', 1)
            }
        elif log_config_from_settings:
            method_log_config = log_config_from_settings
        else:
            from django.conf import settings
            default_api_request_response_log_config = getattr(
                settings, "DEFAULT_API_REQUEST_RESPONSE_LOG_CONFIG", {})

            if default_api_request_response_log_config:
                method_log_config = default_api_request_response_log_config
            else:
                from django_swagger_utils.constants.config import \
                    DEFAULT_API_REQUEST_RESPONSE_LOG_CONFIG
                method_log_config = DEFAULT_API_REQUEST_RESPONSE_LOG_CONFIG

        self.validate_log_config(operation_id, method_log_config)
        return method_log_config

    @staticmethod
    def validate_log_config(operation_id, method_log_config):
        from django_swagger_utils.constants.enums import \
            RequestLogSelector, ResponseLogSelector

        if method_log_config['request_log_selector'] in [
                RequestLogSelector.ENABLE_COMPLETE_LOG.value,
                RequestLogSelector.DISABLE_REQUEST_DATA_ONLY.value
        ] and method_log_config.get(
                "request_response_log_probability", 1) == 0:
            print("Warning: Request log config is enabled for API: {}, "
                  "But Log probability is 0".format(operation_id))

        if method_log_config['response_log_selector'] in [
            ResponseLogSelector.ENABLE_COMPLETE_LOG.value,
            ResponseLogSelector.DISABLE_RESPONSE_DATA_ONLY.value
        ] and method_log_config.get(
                "request_response_log_probability", 1) == 0:
            print("Warning: Response log config is enabled for API: {}, "
                  "But Log probability is 0".format(operation_id))

        return

    @staticmethod
    def get_log_config_from_operation_id_wise_log_config_settings(
        app_name, operation_id
    ):
        from django_swagger_utils.constants.enums import \
            RequestLogSelector, ResponseLogSelector
        from django.conf import settings
        app_operation_id_wise_request_response_config = getattr(
            settings, "APP_OPERATION_ID_WISE_REQUEST_RESPONSE_LOG_CONFIG", {})

        operation_id_wise_request_response_config = \
            app_operation_id_wise_request_response_config.get(app_name)

        request_response_config = None
        if operation_id_wise_request_response_config:
            request_response_config = \
                operation_id_wise_request_response_config.get(operation_id)

        updated_request_response_config = None
        if request_response_config:
            if request_response_config['disable_request_log']:
            ...
            if request_response_config['disable_response_log']:
            ...
            updated_request_response_config = {
                "request_log_selector": request_log_selector,
                "response_log_selector": response_log_selector,
                "request_response_log_probability":
                    request_response_config.get(
                        'request_response_log_probability', 1)
            }

        return updated_request_response_config
