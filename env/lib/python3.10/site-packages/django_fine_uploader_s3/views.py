...
def success_redirect_endpoint(request):
    ...
    if request.method == "POST":
        ...
        temp_link = f"https://{bucket_name}.s3.amazonaws.com/{key_name}"
        if cloud_front:
            temp_link = f"https://{cloud_front}/{key_name}"
        content = {
            "tempLink": temp_link
        }
...
def handle_DELETE(request):
    """Handle file deletion requests using boto3."""
    try:
...
def sign_s3_upload(request):
    ...
    cloud_front = getattr(settings, 'AWS_CLOUDFRONT_DOMAIN', None)
    cloud_front_url = f"https://{bucket_name}.s3.amazonaws.com/{key_name}"
    if cloud_front:
        cloud_front_url = f"https://{cloud_front}/{key_name}"
