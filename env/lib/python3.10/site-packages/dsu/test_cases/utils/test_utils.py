...
class TestUtils:
    ...
    def _add_path_params(self, path_params):
        ...
        print("parent_module_path: ", parent_module_path)
        module = importlib.import_module(parent_module_path)
        self.URL_BASE_PATH = getattr(module, "URL_BASE_PATH", "")

        print("URL_BASE_PATH: ", self.URL_BASE_PATH, module)
        url_suffix = self.URL_SUFFIX.format(**path_params)
        import pkg_resources
        dsu_version = pkg_resources.get_distribution(
            'django-swagger-utils'
        ).version
        from pkg_resources import parse_version

        if parse_version(dsu_version) < parse_version("2.2.12"):
            url = '/api/%s/%s' % (self.APP_NAME, url_suffix)
        else:
            url = '%s%s' % (self.URL_BASE_PATH, url_suffix)
        return url
