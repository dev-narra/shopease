...
from dsu.runtime.security.conditional_decorator import conditional_decorator
from dsu.runtime.security.response_time import response_time
from dsu.runtime.security.wrap_exceptions import wrap_exceptions

logger = logging.getLogger('dsu.debug')


def get_defaults(app_name):
    from dsu.runtime.security.decorator.request_response import \
...
def get_request(request, request_log_selector):
    import json
    from dsu.dsu_gen.openapi.constants.enums import RequestLogSelector
    ...
    if request_data is not None:
        try:
            ...
            if "clientKeyDetailsId" in request_data:
                ...
                from dsu.runtime.security.decorator.drf_json_parser import \
                    drf_json_parser
    ...
    if request_log_selector == \
            RequestLogSelector.DISABLE_REQUEST_DATA_ONLY.value:
        log_data["processed_request_data"] = {}
    else:
        log_data["processed_request_data"] = mask_sensitive_info(request_data)
...
def request_response(
    ...
    def decorator(function):
        ...
        def handler(*args, **kwargs):
            ...
            from dsu.runtime.security.request_response_wrapper import \
...
def log_request_data(args, request_log_selector):
    request = args[0]
    ...
    if getattr(settings, 'LOG_DETAILED_REQUEST_RESPONSE', True):
        if request_log_selector in [
            ...
            request_dict = get_request(request, request_log_selector)

            if getattr(settings, "REQUEST_RESPONSE_LOG_SIZE_LIMIT", True):
                from dsu.runtime.security.decorator.handle_log_limit_response_log import \
                    handle_8kb_log_limit_response_log
                handle_8kb_log_limit_response_log(request_dict)
            else:
                import json
                api_request_log_message = json.dumps(request_dict)
                logger.debug(api_request_log_message)
